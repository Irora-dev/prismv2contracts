...
// `Deploy.s.sol` takes MIN_SEED_PRISM as an override that may only ever RAISE its internal bar — but
// both generators used to emit exactly that internal bar, `(float * 90) / 100`, which makes the knob
...
//
// Compute the ceiling explicitly. This was `-((-x) / Q96)`, which is the standard ceiling-division trick
// for languages with FLOOR division — but BigInt truncates toward zero, so for a positive numerator the
// two negations cancel and it returns the floor. It read as a ceiling and behaved as a floor, leaving the
// deposit 1 wei low: the header below then told the operator to expect 1000000005 wei left in the hook
// while the chain leaves 1000000004, and `LAUNCH.md` step 7 asks them to compare that very number by eye
// in the last read-only step before renouncing.
...
#
# Set RPC_URL and ETH_RPC_URL yourself — they are your own infrastructure.
